...
def handle_frame(arrayBuffer):
    ...
    if attendance_data:
        try:
            ...
            if image is None:
                emit("status", {"message": "Unsupported image format"})
                return

            rgb_frame = image[:, :, ::-1]  # Convert BGR to RGB for face_recognition
            ...
            emit("status", {"message": f"Error processing frame: {str(e)}"})
